...
void build(auto &arr) {
	if (_p + 1 > _nn.size()) {
		...
		if (4 * arr.size() > _nn[_p])
			t[_p].resize(4 * arr.size());
		_nn[_p] = (int)arr.size();
	}
	__nn = _nn.size();
...
//assert(l>=tl && r<=tr);
	...
	if (l == tl && tr == r) {//setVertex
		...
		// assert(t[_p][v * 2].lazy == 0);
		// assert(t[_p][v * 2 + 1].lazy == 0);
		t[_p][v] = merge(t[_p][v * 2] , t[_p][v * 2 + 1]);
	}
}
...
//tested: https://codeforces.com/contest/1557/submission/236625862 (n=1.2e6 and 4 variables in struct with thrice calling, took 2.3sec)


]]></content>
	<!-- Optional: Set a tabTrigger to define how to trigger the snippet -->
	<tabTrigger>range set update range max query segment tree (tested)</tabTrigger>

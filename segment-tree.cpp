...
//imp bug/tip: if you start from update() instead of build(), then __n[_p] is not updated.
...
int joinDeltas(int oldDelta, int newDelta) {
	...
	else if (_p == 1) { //for range assignment update//also add if(t[_p][v].lazy!=NEUTRAL_DELTA) in (whole) 			push
		return newDelta;
	}
}
